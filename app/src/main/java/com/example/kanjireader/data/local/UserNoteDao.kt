...
interface UserNoteDao {
    ...
    @Query("DELETE FROM user_notes WHERE character = :targetCharacter")
    suspend fun deleteNote(targetCharacter: String)

    @Query("DELETE FROM found_sentences WHERE characterOwner = :targetCharacter")
    suspend fun deleteSentencesForCharacter(targetCharacter: String)
